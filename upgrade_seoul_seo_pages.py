...
# images/main — 지역별 작업 사진(랜덤)
# images/cases — 작업후기(랜덤, 번호 쌍 미사용 — 정리 후 쌍 매칭으로 전환 예정)
...
case_templates = [    "{name} 현장에서는 먼저 가족분과 보관할 물품의 기준을 정한 뒤 서류와 사진, 귀중품을 확인했습니다. 이후 생활용품과 폐기물을 구분해 정리 범위를 나누고, 현장 동선에 맞춰 작업을 진행했습니다.",
...
def pick_case_review(idx, count=2):
    rng = random.Random(2000 + idx)
    pool = list(range(1, CASES_MAX + 1))
    return rng.sample(pool, count), rng.sample(pool, count)


def case_gallery_html(name, before_nums, after_nums):
    before = "\n".join(
        f'            <img src="images/cases/before-{three(n)}.jpg" alt="{name} 유품정리 작업후기 작업 전 사진 {i + 1}">'
        for i, n in enumerate(before_nums)
    )
    after = "\n".join(
        f'            <img src="images/cases/after-{three(n)}.jpg" alt="{name} 유품정리 작업후기 작업 후 사진 {i + 1}">'
        for i, n in enumerate(after_nums)
    )
    return f'''<div class="casePhotos"><div class="photoBlock"><h3>작업 전</h3><div class="caseGallery">
{before}
          </div></div><div class="photoBlock"><h3>작업 후</h3><div class="caseGallery">
{after}
          </div></div></div>'''
...
def make_page(name, slug, feature, cost_note, idx):
    ...
    case_before_nums, case_after_nums = pick_case_review(idx)
    case_gallery = case_gallery_html(name, case_before_nums, case_after_nums)
    before = "\n".join(
...
<section id="case" class="areaBox"><div class="container"><h2 class="sectionTitle">{name} 유품정리 작업후기</h2><p class="sectionLead">{case_text}</p>{case_gallery}<div class="notice">현장마다 가족이 보관하고 싶은 물품, 폐기해야 할 물품, 확인이 필요한 서류가 다르기 때문에 상담 단계에서 기준을 먼저 정하는 것이 중요합니다.</div></div></section>
...
<section id="areas" class="areaBox"><div class="container"><h2 class="sectionTitle">{name} 인근 지역 상담 안내</h2><p class="sectionLead">{name} 외에도 서울 주요 지역 유품정리 상담을 함께 안내합니다.</p><div class="areaLinks">
{make_links(name)}
        </div></div></section>
...
'''
    ...
    print("서울 25개 구 SEO 강화 페이지 생성 완료")
